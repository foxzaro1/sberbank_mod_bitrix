...
// }
    $response = $rbs->register_order($order_number, $amount, $return_url, $currency, $gate_comment, $arFiscal);
    ...
    if (in_array($response['errorCode'], array(999, 1, 2, 3, 4, 5, 7, 8))) {
        ...
        if ($auto_open_form && $curUrl != '/personal/orders/') {
            if ($request->get('ORDER_ID')) {
                ...
                {
                    if($arProp['CODE']=="UNIQUE_NUMBER_SBERBANK" || $arProp['CODE']=="EMAIL")
                    {
                        $arProps[$arProp['CODE']] = $arProp;
                    }
                }
                // old version
                //$nowRegisterOrder = $rbs->get_order_status_by_orderNumber($order_number."_0");
                $nowRegisterOrder = $rbs->get_order_status_by_orderNumber($order_number);
                ...
                $arFields = array(
                 ...
                 if (!empty($arBasketList) && is_array($arBasketList))
                 {
                     foreach ($arBasketList as $arItem)
                     {
                         ...
                         foreach ($arSizes as $key => $size) {
                             $arOffers = CCatalogSKU::getOffersList($key);
                             if ($key == $arItem["PRODUCT_ID"] || $arOffers[$key][$arItem["PRODUCT_ID"]]) {
                                 $arOffer3 = $key;
                                 $arItem["PROPERTY_SIZE_FORMATTED"] = $size;
                                 break;
                             }
                         }
                         $measureText = (isset($arItem["MEASURE_TEXT"]) && strlen($arItem["MEASURE_TEXT"])) ? $arItem["MEASURE_TEXT"] : GetMessage("SOA_SHT");
                         $weight = GetWeight($arItem["PRODUCT_ID"], "5_16");
                         if (!$weight) $weight = GetWeight($arItem["PRODUCT_ID"], "11");
                         $add = ($weight) ? "Вес: " . $weight . " кг" : "Размер: ".$arItem["PROPERTY_SIZE_FORMATTED"];
                         $ipropValues = new \Bitrix\Iblock\InheritedProperty\ElementValues(11, ($arOffer3 ? $arOffer3 : $arItem["PRODUCT_ID"]));
                         $arElMetaProp = $ipropValues->getValues();
                         if ($arElMetaProp["ELEMENT_PAGE_TITLE"]) $arItem["NAME"] = $arElMetaProp["ELEMENT_PAGE_TITLE"];
                         $strOrderList .= $arItem["NAME"]." (" . $add . ") "." - ".$arItem["QUANTITY"]." ".$measureText.": ". FormatCurrency($arItem["PRICE"], "RUB") . "/" . $measureText;
                         $strOrderList .= "\n";
                     }
                 }
                 ...
                 $arFields = array(
                    ...
                    "ORDER_USER" => $USER->GetFirstName() . " " . $USER->GetSecondName(),
                    "PRICE" => $checkSumm. " руб.",
                    "BCC" => COption::GetOptionString("sale", "order_email", "order@".$SERVER_NAME),
                    "EMAIL" => (strlen($arProps["EMAIL"]["VALUE"])>0 ? $arProps["EMAIL"]["VALUE"] : $USER->GetEmail()),
                    "ORDER_LIST" => $strOrderList,
                    "SALE_EMAIL" => COption::GetOptionString("sale", "order_email", "order@".$SERVER_NAME),
                    "DELIVERY_PRICE" => $arOrder['PRICE_DELIVERY'],
                    "PAYMENT" => "<a href=\"/sale/payment/result.php?ID=" . $arOrder["ID"]  . "&orderId=".$arProps["UNIQUE_NUMBER_SBERBANK"]["VALUE"]. "\">Счёт на оплату</a>",
                );
                $arFields["TRUE_EVENT"] = "Y";
